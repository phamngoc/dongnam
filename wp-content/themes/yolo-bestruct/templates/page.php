...
if (($layout_style === '') || ($layout_style == '-1')) {
	$layout_style = isset($yolo_bestruct_options['page_layout'])? $yolo_bestruct_options['page_layout'] : 'container';
}
...
if ($page_background_color === '') {
	$page_background_color = $yolo_bestruct_options['page_background_color'];
}
$sidebar = get_post_meta(get_the_ID(), $prefix.'page_sidebar',true);
if (($sidebar === '') || ($sidebar == '-1')) {
	$sidebar = isset($yolo_bestruct_options['page_sidebar']) ? $yolo_bestruct_options['page_sidebar'] : 'right';
}
$left_sidebar = get_post_meta(get_the_ID(), $prefix.'page_left_sidebar',true);
if (($left_sidebar === '') || ($left_sidebar == '-1')) {
	$left_sidebar = isset($yolo_bestruct_options['page_left_sidebar']) ? $yolo_bestruct_options['page_left_sidebar'] : 'sidebar-1';
}
$right_sidebar = get_post_meta(get_the_ID(), $prefix.'page_right_sidebar',true);
if (($right_sidebar === '') || ($right_sidebar == '-1')) {
	$right_sidebar = isset($yolo_bestruct_options['page_right_sidebar']) ? $yolo_bestruct_options['page_right_sidebar'] : 'sidebar-1';
}
$sidebar_width = get_post_meta(get_the_ID(), $prefix.'sidebar_width',true);
if (($sidebar_width === '') || ($sidebar_width == '-1')) {
	$sidebar_width = $yolo_bestruct_options['page_sidebar_width'];
}
$page_comment = isset($yolo_bestruct_options['page_comment']) ? $yolo_bestruct_options['page_comment'] : (comments_open() || get_comments_number());
...
?>
<main class="<?php echo join(' ',$main_class); ?>" style="<?php if( $page_background_color != "" ) : ?>background: <?php echo esc_attr($page_background_color); ?>; <?php endif; ?>">
