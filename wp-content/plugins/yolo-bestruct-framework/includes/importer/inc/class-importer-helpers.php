...
class Yolo_Importer_Helpers {
	...
	private static function get_content_from_url( $url, $file_name = 'Import file' ) {
		...
		if ( empty( $url ) ) {
			return new WP_Error(
				'url_not_defined',
				sprintf(
					__( 'URL for %s%s%s file is not defined!', 'yolo-bestruct' ),
					'<strong>',
					$file_name,
					'</strong>'
				)
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new WP_Error(
				'file_fetching_error',
				sprintf(
					__( 'An error occurred while fetching %s%s%s file from the server!%sReason: %s - %s.', 'yolo-bestruct' ),
					'<strong>',
					$file_name,
					'</strong>',
					'<br>',
	...
	public static function update_json( $content, $name_file, $show_path = false, $file_path = '' ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content, FS_CHMOD_FILE ) ) {
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'yolo-bestruct' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function update_log( $content, $separator_text = '', $file_path = '' ) {
		...
		$time_log  = '+ ' . sprintf( esc_html__( 'Time: %s', 'yolo-bestruct' ), current_time( 'd-m-Y H:i:s' ) ) . PHP_EOL;
	...
	public static function get_contents( $file_path = '' ) {
		...
		if ( ! $data ) {
			...
			Yolo_Importer_Helpers::update_log(
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'yolo-bestruct' ),
					"\n",
					$file_path
				), 
				esc_html__( 'Failed reading file from server!', 'yolo-bestruct' )
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new WP_Error(
				...
				sprintf(
					__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'yolo-bestruct' ),
					'<strong>',
					'</strong>',
		...
		if ( false === ( $creds = request_filesystem_credentials(site_url() . '/wp-admin/', '', false, false, array()) ) ) {
			return new WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'yolo-bestruct' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'yolo-bestruct' )
			);
		}
