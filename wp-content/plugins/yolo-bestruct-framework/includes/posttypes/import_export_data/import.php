 ...
 * @subpackage Importer
 */
if ( !class_exists( 'WP_Importer_Templater' ) ) {
	class WP_Importer_Templater extends WP_Importer {
		...
		public function __construct() {
            $this->dispatch_templater();
        }
		public function dispatch_templater() {
			//$this->header();

			$step = empty( $_GET['step'] ) ? 0 : (int) $_GET['step'];
			switch ( $step ) {
				case 0:
					$this->greet_templater();
					break;
				case 1:
					check_admin_referer( 'import-upload' );
					if ( $this->handle_upload_templater() )
						$this->import_options_templater();
					break;
				case 2:
		...
		function greet_templater() {
			echo '<div class="narrow diepbap">';
			...
			wp_import_upload_form( 'edit.php?post_type=yolo_templater&page=import_data&amp;step=1' );
			echo '</div>';
		}
		...
		function handle_upload_templater() {
		...
		function import_options_templater() {
			$j = 0;
			?>
			<form action="<?php echo admin_url( 'edit.php?post_type=yolo_templater&page=import_data&amp;step=2' ); ?>" method="post">
