...
if ( ! class_exists('Yolo_Framework_Shortcode_Social') ) {
    class Yolo_Framework_Shortcode_Social {
        function __construct() {
            add_shortcode( 'yolo_icon_footer', array($this, 'yolo_icon_footer_shortcode') );
        }

        function yolo_icon_footer_shortcode($atts) {
            $atts  = vc_map_get_attributes( 'yolo_icon_footer', $atts );
            $icon_footer = $layout_type = $el_class = $yolo_animation = $css_animation = $duration = $delay =  '';
            ...
            <?php else : ?>
                <div class="icon_footer-not-select"><?php echo esc_html__( 'Please select icon footer!', 'yolo-bestruct' ); ?></div>
            <?php endif; ?>
        <?php
    ...
    new Yolo_Framework_Shortcode_Social();
}
?>