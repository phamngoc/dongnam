...
function yolo_add_post_columns_head($columns) {
    $columns['post-format'] = esc_html__('Format', 'yolo-bestruct');
    return $columns;
}
...
if (!function_exists('yolo_paging_load_more')) {
	function yolo_paging_load_more() {
		...
		if (!empty($link)) :
			?>
			<button data-href="<?php echo esc_url($link); ?>" type="button"  data-loading-text="<span class='fa fa-spinner fa-spin'></span> <?php esc_html_e("Loading...",'yolo-bestruct'); ?>" class="blog-load-more bestruct-button style1 button-2x" autocomplete="off">
...
if (!function_exists('yolo_post_thumbnail')) {
    function yolo_post_thumbnail($size = '') {
        ...
        $width  = '';
        $height = '';
        $yolo_image_size = yolo_get_image_size($size);
        ...
        switch(get_post_format()) {
            ...
            case 'gallery':
                ...
                $images = yolo_get_post_meta(get_the_ID(), $prefix.'post_format_gallery');
                if (count($images) > 0) {
                ...
                break;
            case 'video':
                $video = yolo_get_post_meta(get_the_ID(), $prefix.'post_format_video');
                if (!is_single()) {
                    $args = array(
                        'size' => $size,
                        'meta_key' => ''
                ...
                break;
            case 'audio':
                $audio = yolo_get_post_meta(get_the_ID(), $prefix.'post_format_audio');
                if (count($audio) > 0) {
    ...
    function yolo_get_image($args) {
        ...
        if ( empty( $image_cache[$key] ) ) {
            ...
            if ((!isset($image_src) || empty($image_src))  && $args['meta_key']) {
                $post_thumbnail_id = yolo_get_post_meta( $args['post_id'], $args['meta_key'], true );
...
if (!function_exists('yolo_get_image_hover')) {
    function yolo_get_image_hover($image,$size, $url, $title, $post_id,$gallery = 0) {
        ...
        if (empty($width) || empty($height)) { // href="%1$s"
            ...
            return sprintf('<div class="entry-thumbnail">
                                <a title="%2$s" class="entry-thumbnail_overlay" href = "%1$s">
                                    <img class="img-responsive" src="%3$s" alt="%2$s"/>
                                </a>
                                <a data-rel="%5$s" href="%4$s" class="prettyPhoto"><i class="fa fa-arrows-alt"></i></a>
                            </div>',
            ...
            <div class="jp-type-playlist">
                <div class="jp-gui jp-interface">
                    <ul class="jp-controls jp-play-pause">
                        <li><a href="javascript:;" class="jp-play" tabindex="1"><i class="fa fa-play-circle-o"></i> <?php esc_html_e('play', 'yolo-bestruct'); ?></a></li>
                        <li><a href="javascript:;" class="jp-pause" tabindex="1"><i class="fa fa-pause"></i> <?php esc_html_e('pause', 'yolo-bestruct'); ?></a></li>
                    </ul>
                    ...
                    <ul class="jp-controls jp-volume">
                        <li>
                            <a href="javascript:;" class="jp-mute" tabindex="1" title="mute"><i class="fa fa-volume-up"></i> <?php esc_html_e('mute', 'yolo-bestruct'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-unmute" tabindex="1" title="unmute"><i class="fa fa-volume-off"></i><?php esc_html_e('unmute', 'yolo-bestruct'); ?></a>
                        </li>
                        <li>
                    ...
                    <ul class="jp-toggles">
                        <li>
                            <a href="javascript:;" class="jp-shuffle" tabindex="1" title="shuffle"><?php esc_html_e('shuffle', 'yolo-bestruct'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-shuffle-off" tabindex="1" title="shuffle off"><?php esc_html_e('shuffle off', 'yolo-bestruct'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-repeat" tabindex="1" title="repeat"><?php esc_html_e('repeat', 'yolo-bestruct'); ?></a>
                        </li>
                        <li>
                            <a href="javascript:;" class="jp-repeat-off" tabindex="1" title="repeat off"><?php esc_html_e('repeat off', 'yolo-bestruct'); ?></a>
                        </li>
                    </ul>
                </div>

                <div class="jp-no-solution">
                    <?php printf(esc_html__('<span>Update Required</span> To play the media you will need to either update your browser to a recent version or update your <a href="%s" target="_blank">Flash plugin</a>.', 'yolo-bestruct'), 'http://get.adobe.com/flashplayer/'); ?>
                </div>
            </div>
        </div>
        <?php
        ...
        <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
            <div id="comment-<?php comment_ID(); ?>" class="comment-body clearfix">
                ...
                <div class="comment-text">
                    <div class="author">
                        <span class="author-name"><?php printf(esc_html__('%s', 'yolo-bestruct'), get_comment_author_link()) ?></span>
                        <span class="comment-meta-date">
                           <span class="time">
                                <?php echo sprintf(esc_html__('%1$s ago', 'yolo-bestruct') , esc_html(yolo_relative_time(get_comment_date('U')))); ?>
                            </span>
                        </span>
                    </div>
                    <div class="text">
                        ...
                        <?php if ($comment->comment_approved == '0') : ?>
                            <em><?php esc_html_e('Your comment is awaiting moderation.', 'yolo-bestruct') ?></em>
                        <?php endif; ?>
                    </div>
                    <div class="comment-meta">
                        <?php comment_reply_link(array_merge($args, array('depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
                        <?php edit_comment_link(esc_html__('Edit', 'yolo-bestruct'), '', '') ?>
                    </div>
                </div>
            </div>
    <?php
    ...
    function yolo_comment_form( $args = array(), $post_id = null ) {
        ...
        <div id="respond-wrap">
            <?php
                ...
                $fields    =  array(
                    'author'        => '<div class="yolo-row"><p class="comment-form-author  yolo-sm-6"><input id="author" name="author" type="text" placeholder="' . esc_html__( 'Enter Your Name*', 'yolo-bestruct' ) . '" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
                    'email'         => '<p class="comment-form-email yolo-sm-6"><input id="email" name="email" type="text" placeholder="' . esc_html__( 'Enter Your Email*', 'yolo-bestruct' ) . '" class="form-control" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
                    'comment_field' => '<div class="yolo-sm-12"><p class="comment-form-comment"><textarea class="form-control" placeholder="' . esc_html__( 'Enter Your Comment', 'yolo-bestruct' ) . '" id="comment" name="comment" cols="40" rows="6" aria-required="true"></textarea></p></div></div>'
                );
                $comments_args = array(
                        'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
                        'logged_in_as'         => '<p class="logged-in-as">' . sprintf( wp_kses(__( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'yolo-bestruct' ), yolo_allowed_tags()), esc_url(admin_url( 'profile.php' )), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
                        'title_reply'          => sprintf('<span>%s</span>', esc_html__( 'Leave your thought', 'yolo-bestruct' )),
                        'title_reply_to'       => sprintf('<span>%s</span>', esc_html__( 'Leave a reply to %s', 'yolo-bestruct' )),
                        'cancel_reply_link'    => esc_html__( 'Click here to cancel the reply', 'yolo-bestruct' ),
                        'comment_notes_before' => '',
                        'comment_notes_after'  => '',
                        'label_submit'         => esc_html__( 'Post Comment', 'yolo-bestruct' ),
                        'comment_field'        => '',
                        'must_log_in'          => ''
                );
                if(is_user_logged_in()) {
                    $comments_args['comment_field'] = '<p class="comment-form-comment"><textarea class="form-control" placeholder="' . esc_html__( 'Enter Your Comment', 'yolo-bestruct' ) . '" id="comment" name="comment" cols="40" rows="6" aria-required="true"></textarea></p>';
                }
