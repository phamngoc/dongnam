...
if ( $filters_count ) {
    ...
    $results_bar_buttons['filters'] = array(
        ...
        'title' => sprintf( esc_html__( 'Filters active %s(%s)%s', 'yolo-bestruct' ), '<span>', $filters_count, '</span>' )
    );
}
...
if ( ! empty( $_REQUEST['s'] ) ) { // Is search query set and not empty?
    ...
    $results_bar_buttons['search_taxonomy'] = array(
        ...
        'title' => sprintf( esc_html__( 'Search results for %s&ldquo;%s&rdquo;%s', 'yolo-bestruct' ), '<span>', esc_html( $_REQUEST['s'] ), '</span>' )
    );
}
// Tag
    ...
    $results_bar_buttons['search_taxonomy']['title'] = sprintf( esc_html__( 'Products tagged: %s&ldquo;%s&rdquo;%s', 'yolo-bestruct' ), '<span>', esc_html( $current_term->name ), '</span>' );
}
