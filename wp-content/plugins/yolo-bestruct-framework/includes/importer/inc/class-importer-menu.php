	...
	public static function menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			...
			Yolo_Importer_Helpers::update_log(
				...
				sprintf(
					esc_html__( 'Menu item skipped: %s.', 'yolo-bestruct' ),
					esc_html(
						$menu_slug
					)
				)
	...
	public static function nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
	    ...
	    if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) ) {
			...
			Yolo_Importer_Helpers::update_log(
				...
				sprintf(
					esc_html__( 'Update Nav Menu Item Failed: %s.', 'yolo-bestruct' ),
					esc_html(
						$menu_id
					)
				)
	    ...
	    if ( ! $menu && 0 !== $menu_id ) {
			...
			Yolo_Importer_Helpers::update_log(
				...
				sprintf(
					esc_html__( 'Invalid Menu ID: %s.', 'yolo-bestruct' ),
					esc_html(
						$menu_id
					)
				)
	    ...
	    if ( is_wp_error( $menu ) ) {
			...
			Yolo_Importer_Helpers::update_log(
				'', 
				sprintf(
					esc_html__( 'ERROR Create Menu: %s.', 'yolo-bestruct' ),
					esc_html(
						$menu
					)
				)
	    ...
	    if ( ! $update ) {
	        ...
	        if ( ! $menu_item_db_id || is_wp_error( $menu_item_db_id ) ) {
				...
				Yolo_Importer_Helpers::update_log(
					'', 
					sprintf(
						esc_html__( 'ERROR Update Menu: %s.', 'yolo-bestruct' ),
						esc_html(
							$menu_item_db_id
						)
					)
		...
		Yolo_Importer_Helpers::update_log(
			'', 
			sprintf(
				esc_html__( 'Success Update Menu: %s+Menu name: %s%s+Menu ID: %s.', 'yolo-bestruct' ),
				"\n",
