...
function yolo_generate_less()
{
    try {
        ...
        $home_preloader =  $yolo_bestruct_options['home_preloader'];
        $css_variable   = yolo_custom_css_variable();
        $custom_css     = yolo_custom_css();
        // For DEV: build variable to style. Remove when complete
        ...
        // old if (!$wp_filesystem->put_contents( get_template_directory().   "/assets/css/custom-style.css", $css, FS_CHMOD_FILE))
        // Custom style will be build in upload/yolo-custom-css
        $css_dir = yolo_create_upload_dir( $wp_filesystem );
        if (!$wp_filesystem->put_contents( $css_dir ."/custom-style.css", $css, FS_CHMOD_FILE)) {
            return array(
                'status' => 'error',
                'message' => esc_html__( 'Could not save file', 'yolo-bestruct' )
            );
        }
