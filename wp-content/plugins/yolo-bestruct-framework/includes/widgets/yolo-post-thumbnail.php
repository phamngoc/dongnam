...
class Yolo_Post_Thumbnail_Widget extends Yolo_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( "Widget show post with thumbnail.", 'yolo-bestruct' );
		$this->widget_id          = 'yolo_widget_post_thumbnail';
		$this->widget_name        = esc_html__( 'YOLO Post Thumbnail', 'yolo-bestruct' );
		$this->cached = false;
		$categories = array();
		...
		$this->settings           = array(
				...
				'style' => array(
					'type' => 'select',
					'std'  => '',
					'label'=> esc_html__('Style', 'yolo-bestruct'),
					'options' => array(
							'full_width'   => esc_html__( 'Full-width', 'yolo-bestruct' ),
							'size-thumbnail'   => esc_html__( 'Thumbnail', 'yolo-bestruct' ),
							'date'   => esc_html__( 'Date', 'yolo-bestruct' )
					)
				),
				'posts_per_page' => array(
					...
					'max'   => '',
					'std'   => 5,
					'label' => esc_html__( 'Number of posts to show', 'yolo-bestruct' )
				),
				'orderby' => array(
					'type'  => 'select',
					'std'   => 'date',
					'label' => esc_html__( 'Order by', 'yolo-bestruct' ),
					'options' => array(
							'latest'   => esc_html__( 'Latest', 'yolo-bestruct' ),
							'popular'   => esc_html__( 'Popular', 'yolo-bestruct' ),
							'comment'  => esc_html__( 'Most Commented', 'yolo-bestruct' ),
					)
				),
				'categories' => array(
						'type'  => 'multi-select',
						'std'   => '',
						'label'=>esc_html__('Categories','yolo-begreen'),
						'desc' => esc_html__( 'Select a category or leave blank for all', 'yolo-begreen' ),
						'options' => $categories_options,
				),
				'hide_author' => array(
						'type'  => 'checkbox',
						'std'   => 0,
						'label' => esc_html__( 'Hide author in post meta info', 'yolo-bestruct' )
				),
				'hide_date' => array(
						'type'  => 'checkbox',
						'std'   => 0,
						'label' => esc_html__( 'Hide date in post meta info', 'yolo-bestruct' )
				),
				'hide_comment' => array(
						'type'  => 'checkbox',
						'std'   => 0,
						'label' => esc_html__( 'Hide comment in post meta info', 'yolo-bestruct' )
				),
		);
		parent::__construct();
	}
				...
				while ($r->have_posts()): 
					...
					echo '<div class="posts-thumbnail-content">';
						...
						if(!$hide_comment){
							...
							if ( $number > 1 ) {
								$output = str_replace( '%', number_format_i18n( $number ), ( false === false ) ? esc_html__( '%', 'yolo-bestruct' ) : false );
							} elseif ( $number == 0 ) {
								$output = ( false === false ) ? esc_html__( '0', 'yolo-bestruct' ) : false;
							} else { // must be one
								$output = ( false === false ) ? esc_html__( '1', 'yolo-bestruct' ) : false;
							}
