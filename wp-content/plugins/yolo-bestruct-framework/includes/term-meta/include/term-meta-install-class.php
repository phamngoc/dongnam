<?php
	 ...
	 *
	 * - Creates & maintains the `wp_yolo_termmeta` table
	 * - Listens for new blogs
	 * - Listens for blog deletion and deletes the `wp_yolo_termmeta` table
	 * - Uses `WP_Meta_Query` and looks for    `meta_query` arguments
	 * - Deletes all metadata for terms when terms are deleted
	 * - Adds `wp_yolo_termmeta` to the main database object when appropriate
	 * -
	 *
	...
	if ( ! class_exists( 'Yolo_Term_Meta_Install' ) ):

		final class Yolo_Term_Meta_Install {
			...
			public function __construct() {
				...
				// Setup plugin
				$this->file     = PLUGIN_YOLO_BESTRUCT_FRAMEWORK_FILE;
				$this->url      = PLUGIN_YOLO_BESTRUCT_FRAMEWORK_URL;
				$this->path     = PLUGIN_YOLO_BESTRUCT_FRAMEWORK_DIR;
				$this->basename = PLUGIN_YOLO_BESTRUCT_FRAMEWORK_NAME;
				$this->db       = $wpdb;
			...
			private function maybe_upgrade_database() {
				...
				$yolo_term_meta_table_installed = (bool) get_option( 'yolo_term_meta_table_installed', FALSE );
				// Needs
				if ( ! $yolo_term_meta_table_installed ) {
					$this->activate();
					update_option( 'yolo_term_meta_table_installed', TRUE );
				}
			}
			...
			public function add_termmeta_to_db_object() {
				global $wpdb;
				$this->db->yolo_termmeta = $wpdb->yolo_termmeta = "{$this->db->prefix}yolo_termmeta";
			}
			...
			public function drop_tables( $tables = array() ) {
				...
				// Table to check for
				$table = "{$this->db->prefix}yolo_termmeta";
			...
			/**
			 * Create the `yolo_termmeta` table
			 *
			 * @since 0.1.0
			 */
			private function create_termmeta_table() {
				...
				dbDelta( array(
					        "CREATE TABLE {$this->db->prefix}yolo_termmeta (
								meta_id bigint(20) NOT NULL AUTO_INCREMENT,
								term_id bigint(20) NOT NULL,
								yolo_term_id bigint(20) NOT NULL,
			...
			public function delete_all_meta_for_term( $term_id = 0 ) {
				...
				$query         = "SELECT meta_id FROM {$this->db->yolo_termmeta} WHERE term_id = %d";
				...
				foreach ( $term_meta_ids as $mid ) {
					delete_metadata_by_mid( 'yolo_term', $mid );
				}
			}
		}
	endif; // class_exists( 'Yolo_Term_Meta_Install' )

	if ( ! function_exists( 'Yolo_Term_Meta_Install' ) ):
		 ...
		 * @since 0.1.0
		 */
		function Yolo_Term_Meta_Install() {
			new Yolo_Term_Meta_Install();
		}

		add_action( 'plugins_loaded', 'Yolo_Term_Meta_Install' );

	endif; // function_exists( 'Yolo_Term_Meta_Install' )