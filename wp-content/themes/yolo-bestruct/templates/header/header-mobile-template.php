...
if (isset($yolo_bestruct_options['mobile_header_layout']) && !empty($yolo_bestruct_options['mobile_header_layout'])) {
    $mobile_header_layout = $yolo_bestruct_options['mobile_header_layout'];
}

$header_class[] = $mobile_header_layout;

// Get logo url for mobile
if (isset($yolo_bestruct_options['mobile_header_logo']['url']) && !empty($yolo_bestruct_options['mobile_header_logo']['url'])) {
	$logo_url = $yolo_bestruct_options['mobile_header_logo']['url'];
}else if (isset($yolo_bestruct_options['logo']['url']) && !empty($yolo_bestruct_options['logo']['url'])) {
...
$mobile_header_shopping_cart = $yolo_bestruct_options['mobile_header_shopping_cart'];

$mobile_header_search_box = $yolo_bestruct_options['mobile_header_search_box'];

$mobile_header_menu_drop = 'dropdown';

if (isset($yolo_bestruct_options['mobile_header_menu_drop']) && !empty($yolo_bestruct_options['mobile_header_menu_drop'])) {
	$mobile_header_menu_drop = $yolo_bestruct_options['mobile_header_menu_drop'];
}

$header_container_wrapper_class = array('yolo-header-container-wrapper', 'menu-drop-' . $mobile_header_menu_drop);

$mobile_header_stick = isset($yolo_bestruct_options['mobile_header_stick']) ? $yolo_bestruct_options['mobile_header_stick'] : '0';
...
if (isset($yolo_bestruct_options['logo_mobile_height']) && isset($yolo_bestruct_options['logo_mobile_height']['height']) && ! empty($yolo_bestruct_options['logo_mobile_height']['height'])) {
	$logo_mobile_height = $yolo_bestruct_options['logo_mobile_height']['height'];
