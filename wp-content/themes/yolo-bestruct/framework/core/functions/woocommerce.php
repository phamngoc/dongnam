...
if ( class_exists( 'WooCommerce' ) ) {
    ...
    if ( ! function_exists('yolo_woocommerce_sale_flash') ) {
        function yolo_woocommerce_sale_flash($sale_flash,$post,$product) {
            ...
            $product_sale_flash_mode = isset($yolo_bestruct_options['product_sale_flash_mode']) ? $yolo_bestruct_options['product_sale_flash_mode'] : '' ;
            if ($product_sale_flash_mode == 'percent') {
                ...
                if ($sale_percent > 0) {
                    return '<span class="on-sale product-flash">' . $sale_percent . '%</span>' . '<span class="on-sale product-flash">' . esc_html__( 'Sale', 'yolo-bestruct' ) . '</span>';
                } else {
                    return "";
                }
    ...
    function yolo_get_rating_html($rating_html, $rating) {
        global $yolo_bestruct_options;
        $rating_class = '';
        $rating_option = $yolo_bestruct_options['product_show_rating']? $yolo_bestruct_options['product_show_rating'] : true;
        if( $rating_option == false ) {
            $rating_class = 'rating-invisible';
        }
        if ( $rating > 0 ) {
            $title = sprintf( esc_html__( 'Rated %s out of 5', 'yolo-bestruct' ), $rating );
        } else {
            $title = 'Not yet rated';
            $rating = 0;
        }
        ...
        $rating_html .= '<span style="width:' . ( ( $rating / 5 ) * 100 ) . '%"><strong class="rating">' . $rating . '</strong> ' . esc_html__( 'out of 5', 'yolo-bestruct' ) . '</span>';
        $rating_html .= '</div>';
    ...
    function yolo_woocommerce_wishlist(){
        ...
        <a title="<?php esc_html_e( 'Wishlist','yolo-bestruct' ); ?>" href="<?php echo esc_url($wishlist_page); ?>" class="yolo-wishlist">
    ...
    if ( ! function_exists( 'yolo_admin_style_attributes_types' ) ) :
        ...
        function yolo_admin_style_attributes_types( $current ) {
            ...
            $current[ 'color' ] = esc_html__( 'Color', 'yolo-bestruct' );
            $current[ 'image' ] = esc_html__( 'Image', 'yolo-bestruct' );
            $current[ 'label' ] = esc_html__( 'Label', 'yolo-bestruct' );
    ...
    if ( ! function_exists( 'yolo_admin_style_attributes_values' ) ) :
        ...
        function yolo_admin_style_attributes_values( $tax, $i ) {
            ...
            if ( in_array( $tax->attribute_type, array( 'color', 'image', 'label' ) ) ) {
                ...
                ?>
                <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'yolo-bestruct' ); ?>"
                ...
                <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'yolo-bestruct' ); ?></button>
                <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'yolo-bestruct' ); ?></button>
                <button class="button fr plus add_new_attribute"><?php esc_html_e( 'Add new', 'yolo-bestruct' ); ?></button>
                <?php
            }
        }
    ...
    if ( ! function_exists( 'yolo_wc_color_variation_attribute_options' ) ) :
        ...
        function yolo_wc_color_variation_attribute_options( $args = array() ) {
            ...
            $args = wp_parse_args( $args, array(
                ...
                'id'               => '',
                'class'            => '',
                'show_option_none' => esc_html__( 'Choose an option', 'yolo-bestruct' )
            ) );
                ...
                if ( $product && taxonomy_exists( $attribute ) ) {
                    ...
                    foreach ( $terms as $term ) {
                        if ( in_array( $term->slug, $options ) ) {
                            ...
                            ?>
                            <li data-toggle="tooltip" data-placement="top"
                                class="variable-item image-variable-item image-variable-item-<?php echo esc_attr($term->slug) ?> <?php echo esc_attr($selected_class) ?>"
                ...
                if ( $product && taxonomy_exists( $attribute ) ) {
                    ...
                    foreach ( $terms as $term ) {
                        if ( in_array( $term->slug, $options ) ) {
                            ...
                            ?>
                            <li data-toggle="tooltip" data-placement="top"
                                class="variable-item label-variable-item label-variable-item-<?php echo esc_attr($term->slug) ?> <?php echo esc_attr($selected_class) ?>"
                                title="<?php echo esc_html( $term->name ) ?>"
    ...
    if (!function_exists('yolo_show_products_per_page')) {
        function yolo_show_products_per_page() {
            global $yolo_bestruct_options;
            $product_per_page = $yolo_bestruct_options['product_per_page'];
    ...
    if (!function_exists('yolo_related_products_args')) {
        function yolo_related_products_args() {
	        global $yolo_bestruct_options;
	        $args['posts_per_page'] = isset($yolo_bestruct_options['related_product_count']) ? $yolo_bestruct_options['related_product_count'] :  8;
	        return $args;
        }
	...
	if (!function_exists('yolo_woocommerce_product_related_posts_relate_by_category')) {
		function yolo_woocommerce_product_related_posts_relate_by_category() {
			global $yolo_bestruct_options;
			return $yolo_bestruct_options['related_product_condition']['category'] == 1 ? true : false;
		}
	...
	if (!function_exists('yolo_woocommerce_product_related_posts_relate_by_tag')) {
		function yolo_woocommerce_product_related_posts_relate_by_tag() {
			global $yolo_bestruct_options;
			return $yolo_bestruct_options['related_product_condition']['tag'] == 1 ? true : false;
		}
    ...
    if (!function_exists('yolo_button_continue_shopping')) {
        function yolo_button_continue_shopping () {
            ...
            ?>
            <a href="<?php echo esc_url($continue_shopping); ?>" class="continue-shopping button"><?php esc_html_e( 'Continue shopping', 'yolo-bestruct' ); ?></a>
            <?php
        }
    }
    ...
    if (!function_exists('yolo_shop_page_content')) {
        ...
        function yolo_shop_page_content() {
            global $yolo_bestruct_options;
            $show_page_shop_content = isset($yolo_bestruct_options['show_page_shop_content']) ? $yolo_bestruct_options['show_page_shop_content'] : '0';
        ...
        $show_page_shop_content = isset($yolo_bestruct_options['show_page_shop_content']) ? $yolo_bestruct_options['show_page_shop_content'] : '';
    ...
    ================================================== */
    $product_add_to_cart = isset($yolo_bestruct_options['product_add_to_cart']) ? $yolo_bestruct_options['product_add_to_cart'] : true;
    ...
    if (!function_exists('yolo_woocommerce_add_custom_general_fields')) {
        function yolo_woocommerce_add_custom_general_fields() {
            ...
            woocommerce_wp_checkbox(
                array(
                    'id'    => 'yolo_product_new',
                    'label' => esc_html__( 'Product New', 'yolo-bestruct' )
                )
            );
            woocommerce_wp_checkbox(
                array(
                    'id'    => 'yolo_product_hot',
                    'label' => esc_html__( 'Product Hot', 'yolo-bestruct' )
                )
            );
            echo '</div>';
        }
    ...
    if (!function_exists('yolo_columns_into_product_list')) {
        function yolo_columns_into_product_list($defaults) {
            $defaults['yolo_product_new'] = esc_html__('New','yolo-bestruct');
            $defaults['yolo_product_hot'] = esc_html__('Hot','yolo-bestruct');
            return $defaults;
        }
    ...
    if (!function_exists('yolo_paging_load_more_product')) {
        function yolo_paging_load_more_product($products) {
            ...
            if (!empty($link)) :
                ?> 
                    <button data-href="<?php echo esc_url($link); ?>" type="button"  data-loading-text="<span class='fa fa-spinner fa-spin'></span> <?php esc_html_e( "Loading...",'yolo-bestruct' ); ?>" class="product-load-more bestruct-button style1 button-2x" autocomplete="off">
    ...
    if ( ! function_exists( 'yolo_paging_nav_product' ) ) {
        function yolo_paging_nav_product($products) {
            ...
            $page_links = paginate_links( array(
                ...
                'prev_text' => esc_html__('Prev', 'yolo-bestruct'),
                'next_text' => esc_html__('Next', 'yolo-bestruct'),
                'type'      => 'array'
            ) );
    ...
    function yolo_category_menu() {
       ...
       $output = '<h4 class = "yolo-filter-categories-mobile">'.esc_html__('Categories','yolo-bestruct').'</h4>';
       $output .= '<ul class="yolo-filter-categories">';
       if ( $cat_all ) {
        $output .= '<li class="'.$current_class.'"><a href="' .esc_url($page_shop_url).'">' . esc_html__( 'All', 'yolo-bestruct') . '</a></li>'; // @TODO: change shop to get shop page
       } else {
           $output .= '<li class=""><a href="'.esc_url($page_shop_url).'">' . esc_html__( 'All', 'yolo-bestruct') . '</a></li>';
       }
       if( $categories ){
           foreach ( $categories as $key => $val ) {
               if ( $val->count > 0 ) {
