...
class Yolo_Notice_Install {
	...
	private $product_id           = 'yolo-bestruct';
	...
	public function load_enqueue_script_setup() {
		...
		wp_enqueue_script( 'setup-install-demo', plugins_url( PLUGIN_YOLO_BESTRUCT_FRAMEWORK_NAME.'/admin/assets/js/min/yolo.setup.install.demo.js') );

		wp_enqueue_style( 'setup-style', plugins_url( PLUGIN_YOLO_BESTRUCT_FRAMEWORK_NAME.'/admin/assets/css/yolo-setup.css') );
		...
		wp_localize_script( 'setup-install-demo', 'yoloSetupDemo', 
			array( 
				...
				'img_ajax_load' => plugins_url( PLUGIN_YOLO_BESTRUCT_FRAMEWORK_NAME.'/admin/assets') . '/images/ajax-loader.gif'
			) 
		);
	}
	// -- Tools options
	public static function yolo_import_demo_options() {
		$list_demo = array(
			array(
                'name' => esc_html__( 'Bestruct', 'yolo-bestruct' ),
                'img'  => plugins_url( PLUGIN_YOLO_BESTRUCT_FRAMEWORK_NAME.'/') . 'includes/importer/data-demo/bestruct/screenshot.png',
                'file' => 'bestruct'
            ),
		);
		...
		?>	<!-- <table class="widefat" cellspacing="0" style="width: 99%;">
				<thead>
					<tr>
						<th colspan="1" data-export-label="<?php echo esc_html__( 'Settings', 'yolo-bestruct' ); ?>">
							<label class="hide_main">
								<?php echo esc_html__( 'Settings', 'yolo-bestruct' ); ?>
							</label>
						</th>
					</tr>
				</thead>
				<tbody id="yolo_main_select">
					<tr>
						<td>
							<input type='checkbox' data-id='import_post' id='import_post' value='1' checked /> <?php echo esc_html__( 'Import Post', 'yolo-bestruct' ); ?>
						</td>
					</tr>
					<tr>
						<td>
							<input type='checkbox' data-id='import_nav' id='import_nav' value='1' checked /> <?php echo esc_html__( 'Import Widget', 'yolo-bestruct' ); ?>
						</td>
					</tr>
					<tr>
						<td>
							<input type='checkbox' data-id='import_comment' id='import_comment' value='1' checked /> <?php echo esc_html__( 'Import Option', 'yolo-bestruct' ); ?>
						</td>
					</tr>
					<tr>
						<td>
							<input type='checkbox' data-id='import_revslider' id='import_revslider' value='1' checked /> <?php _e( 'Import Revslider', 'yolo-bestruct' ); ?>
						</td>
					</tr>
				</tbody>
			</table> -->
					...
					<div class="themes">
						<?php foreach ($list_demo as $id => $demo) : ?>
							<div class="theme" tabindex="0">
								...
								<span class="more-details" id="install_<?php echo esc_attr( $demo['file'] ); ?>"><?php esc_html_e( 'Install ' .$demo['name'], 'yolo-bestruct' ); ?></span>
								...
								<div class="theme-actions">
									...
									><?php esc_html_e( 'Install Demo', 'yolo-bestruct' ); ?></button>
								</div>
							</div>
						<?php endforeach; ?>
					</div>
