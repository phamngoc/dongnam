 ...
 */
 class YOLO_WC_Widget_Price_Filter extends Yolo_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Show price range to sort for shop page. This widget use for shop ajax page.', 'yolo-bestruct' );
		$this->widget_id          = 'yolo_woocommerce_price_filter';
		$this->widget_name        = esc_html__( 'YOLO WooCommerce Price Ajax Filter', 'yolo-bestruct' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by price', 'yolo-bestruct' ),
			...
			'price_range_size' => array(
				...
				'max'   => '',
				'std'   => 50,
				'label' => esc_html__( 'Price range size', 'yolo-bestruct' )
			),
			'max_price_ranges' => array(
				...
				'max'   => '',
				'std'   => 10,
				'label' => esc_html__( 'Max price ranges', 'yolo-bestruct' )
			),
			'hide_empty_ranges' => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__( 'Hide empty price ranges', 'yolo-bestruct' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( strlen( $min_price ) > 0 ) {
			$output .= '<li><a href="' . esc_url( $link ) . '">' . esc_html__( 'All', 'yolo-bestruct' ) . '</a></li>';
		} else {
            $output .= '<li class="current">' . esc_html__( 'All', 'yolo-bestruct' ) . '</li>';
		}
