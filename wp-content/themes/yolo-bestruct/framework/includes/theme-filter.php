    ...
    function yolo_comment_fields($fields) {
        ...
        $fields = array(
            'author' => '<div class="form-group col-md-12">' .
                '<input id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" placeholder="'.esc_html__('Name*','yolo-bestruct').'" ' . $aria_req . '>' .
                '</div>',
            'email' => '<div class="form-group col-md-12">' .
                '<input id="email" name="email" ' . ($html5 ? 'type="email"' : 'type="text"') . ' value="' . esc_attr($commenter['comment_author_email']) . '" placeholder="'.esc_html__('Email*','yolo-bestruct').'" ' . $aria_req . '>' .
                '</div>'
        );
...
if (!function_exists('yolo_comment_form_args')) {
    function yolo_comment_form_args($comment_form_args) {
        ...
        $comment_form_args['comment_field'] = '<div class="form-group col-md-12">' .
            '<textarea rows="6" id="comment" name="comment"  placeholder="'.esc_html__('Message*','yolo-bestruct') .'" '. $aria_req .'></textarea>' .
            '</div>';

        $comment_form_args['class_submit'] = 'bestruct-button style1 button-2x';
        $comment_form_args['label_submit'] = esc_html__('Send us now', 'yolo-bestruct');
...
if (!function_exists('yolo_main_menu_one_page_filter')) {
	function yolo_main_menu_one_page_filter($args) {
		...
		$prefix      = 'yolo_';
		$is_one_page_menu = get_post_meta(get_the_ID(),$prefix . 'is_one_page_menu',true);
		if ($is_one_page_menu == '1') {
...
if (!function_exists('yolo_header_customize_filter')) {
	...
	function yolo_header_customize_filter($args) {
		...
		if ($enable_header_customize == '1') {
			...
			if (isset($yolo_bestruct_options['header_customize']) && isset($yolo_bestruct_options['header_customize']['enabled']) && is_array($yolo_bestruct_options['header_customize']['enabled'])) {
				foreach ($yolo_bestruct_options['header_customize']['enabled'] as $key => $value) {
...
if (!function_exists('yolo_less_css_url_rewrite')) {
	function yolo_less_css_url_rewrite() {
		add_rewrite_rule( 'wp-content/themes/bestruct/yolo-less-css', 'index.php', 'top' );
		flush_rewrite_rules();
	}
...
if (!function_exists('yolo_sticky_logo')) {
	function yolo_sticky_logo($agrs){
		$yolo_header_layout = yolo_get_header_layout();
		global $yolo_bestruct_options;
		if (in_array($yolo_header_layout, array('header-3','header-6'))) { // logo in navigation
			return $agrs;
		}
		...
		$logo_sticky_meta = get_post_meta(get_the_ID(),$prefix . 'sticky_logo', 'type=image_advanced',true);
		...
		if (empty($logo_sticky)) {
			...
			else if (isset($yolo_bestruct_options['logo']) && isset($yolo_bestruct_options['logo']['url'])) {
				$logo_sticky = $yolo_bestruct_options['logo']['url'];
			}
		}
