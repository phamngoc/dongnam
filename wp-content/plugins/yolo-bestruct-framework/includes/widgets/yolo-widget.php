 ...
 * @extends     WP_Widget
 */
abstract class Yolo_Widget extends WP_Widget {
	...
	function get_cached_widget( $args ) {
		...
		$cache = wp_cache_get( apply_filters( 'yolo_cached_widget_id', $this->widget_id ), 'widget' );
	...
	public function cache_widget( $args, $content ) {
		...
		wp_cache_set( apply_filters( 'yolo_cached_widget_id', $this->widget_id ), $cache, 'widget' );
	}
	...
	public function flush_widget_cache() {
		wp_cache_delete( apply_filters( 'yolo_cached_widget_id', $this->widget_id ), 'widget' );
	}
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			break;
                ...
                case "multi-select" :
                    ...
                    <p>
                        <label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html($setting['label']); ?></label>
                        <input name="<?php echo esc_attr($this->get_field_name( $key )); ?>" type="hidden" value="<?php if(!is_array($value)) echo esc_attr( $value ); ?>" />
                        <select multiple class="widefat widget-select2" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" data-value="<?php if(!is_array($value)) echo esc_attr( $value ); ?>">
                            <?php foreach ( $setting['options'] as $option_key => $option_value ) : ?>
				...
				case "icon":
					...
					<div style="margin: 13px 0">
						...
						<div>
							...
							<input style="float: right"  title="<?php echo esc_html__('Click to browse icon','yolo-bestruct') ?>" class="browse-icon button-secondary" type="button" value="<?php echo esc_html__('Browse...','yolo-bestruct') ?>" />
				...
				case "image":
					...
					<div style="margin: 13px 0">
						...
						<div class="widget-image-field">
							...
							<button style="float: right" title="<?php echo esc_html__('Click to browse image','yolo-bestruct') ?>" class="browse-images button-secondary" type="button"><?php echo esc_html__('Browse...','yolo-bestruct') ?></button>
						</div>
						<script type="text/javascript">
							//<![CDATA[
							jQuery(document).ready(function() {
								yolo_media_init("#<?php echo esc_attr( $this->get_field_id( $key ) ); ?>",'.browse-images');
							});
							//]]>
						</script>
					</div>
